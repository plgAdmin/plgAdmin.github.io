...
function dhtmlXForm(parentObj, data, skin) {
	...
	this.skin = (skin || window.dhx4.skin || (typeof(dhtmlx) != "undefined" ? dhtmlx.skin : null) || window.dhx4.skinDetect(
		"dhx_form") || "material");
	...
	this._prepare = function(ofsLeft, pos) {
		...
		if (this.b_index == null) this.b_index = 0;
		else this.b_index++;
		...
		if (typeof(ofsLeft) != "undefined") this.base[this.b_index].style.cssText += " margin-left:" + ofsLeft +
			"px!important;";
	...
	this._addItem = function(type, id, data, sId, lp, pos, insertAfter) {
		...
		if (type == "list" && lp != null && this.itemPull[this.idPrefix + lp] != null && typeof(this.itemPull[this.idPrefix +
				lp]._addSubListNode) == "function") {
		...
		// validate
		if (data.validate) {
			if (typeof(data.validate != "undefined") && (typeof(data.validate) == "function" || typeof(window[data.validate]) ==
					"function")) {
	...
	*********************************************************************************************************************************************/
	...
	this._initObj = function(data, url) {
		...
		for (var q = 0; q < data.length; q++) {
			...
			if (typeof(data[q]) != "undefined" && data[q].type == "settings")
				for (var a in data[q]) this.idef[a] = data[q][a];
		}
			...
			for (var q = 0; q < xmlStruct.childNodes.length; q++) {
				...
				if (typeof(xmlStruct.childNodes[q].tagName) != "undefined") {
					...
					if (this._xmlSubItems[tg] != null) {
						...
						xn = null;
						if (rootLevel) data.push(k);
						else data[node].push(k);
	...
	*********************************************************************************************************************************************/
	...
	this.doWithItem = function(id, method, a, b, c, d) {
		...
		if (typeof(id) == "object") {
			...
			for (var k in this.itemPull) {
				if ((this.itemPull[k]._value == value || value === null) && this.itemPull[k]._group == group) return this.objPull[
					k][method](this.itemPull[k], a, b, c, d);
	...
	this._removeItem = function(id, value) {
		if (value != null) id = this.doWithItem([id, value], "destruct");
		else this.doWithItem(id, "destruct");
	...
	this.setItemValue = function(id, value) {
		...
		if (this.getItemType(id) == "radio") {
			if (this._getRGroup(id, value) != null) this.checkItem(id, value);
			else this.uncheckItem(id, this.getCheckedValue(id));
			return null;
		}
	...
	this.clear = function() {
		...
		for (var a in this.itemPull) {
			...
			// nested lists
			if (this.itemPull[a]._list)
				for (var q = 0; q < this.itemPull[a]._list.length; q++) this.itemPull[a]._list[q].clear();
	...
	for (var a in this.items) {
		...
		if (!this.items[a].show) {
			this.items[a].show = function(item) {
				item.style.display = "";
				if (item._listObj)
					for (var q = 0; q < item._listObj.length; q++) item._listObj[q].show(item._listBase[q]);
			}
		}
		...
		if (!this.items[a].hide) {
			this.items[a].hide = function(item) {
				...
				if (item._listObj)
					for (var q = 0; q < item._listObj.length; q++) item._listObj[q].hide(item._listBase[q]);
			}
		}
		...
		if (!this.items[a]._validate) {
			this.items[a]._validate = function(item) {
				...
				if (!(item.callEvent("onValidate" + (r ? "Success" : "Error"), [item._idd, val, r]) === false)) item.getForm().setValidateCss(
					item._idd, r);
...
dhtmlXForm.prototype.items.checkbox = {
	...
	doAddLabel: function(item, data) {
		...
		t.innerHTML = "<div class='dhxform_label_nav_link' " +
			...
			"role='link' tabindex='0'>" + data.label + (data.info ? "<span class='dhxform_info'>[?]</span>" : "") + (item._required ?
				"<span class='dhxform_item_required'>*</span>" : "") + '</div>';
	...
	doAddInput: function(item, data, el, type, pos, dim, css) {
		...
		if (dimFix && ({
			...
			if (dhtmlXForm.prototype.items[this.t]._dim == null) dhtmlXForm.prototype.items[this.t]._dim = item.getForm()._checkDim(
				p, t);
	...
	doAttachEvents: function(item) {
		...
		item.childNodes[item._ll ? 1 : 0][window.dhx4.isIPad ? "ontouchstart" : "onmousedown"] = function(e) {
			...
			if (!this.parentNode._enabled || this.parentNode._ro || (typeof(t.className) != "undefined" && t.className ==
					"dhxform_note")) {
				e.cancelBubble = true;
				if (e.preventDefault) e.preventDefault();
	...
	doClick: function(item) {
		...
		if (item.checkEvent("onBeforeChange"))
			if (item.callEvent("onBeforeChange", [item._idd, item._value, item._checked]) !== true) return;
	...
	setChecked: function(item, state) {
		...
		item.childNodes[item._ll ? 1 : 0].lastChild.className = item.childNodes[item._ll ? 1 : 0].lastChild.className.replace(
			/chbx[0-1]{1}/gi, "") + (item._checked ? " chbx1" : " chbx0");
	...
	setText: function(item, text) {
		item.childNodes[item._ll ? 0 : 1].childNodes[0].innerHTML = text + (item._required ?
			"<span class='dhxform_item_required'>*</span>" : "");
	},

	getText: function(item) {
		return item.childNodes[item._ll ? 0 : 1].childNodes[0].innerHTML.replace(
...
dhtmlXForm.prototype.items.radio = {
	...
	doClick: function(item) {
		...
		if (item.checkEvent("onBeforeChange"))
			if (item.callEvent("onBeforeChange", args) !== true) return;
		this.setChecked(item, true);
...
dhtmlXForm.prototype.items.select = {
		...
		window.dhx4.ajax.get(url, function(r) {
			...
			if (t.indexOf("{") === 0) {
				...
				for (var q = 0; q < root.childNodes.length; q++) {
					if (typeof(root.childNodes[q].tagName) != "undefined" && String(root.childNodes[q].tagName).toLowerCase() ==
						"item") {
			...
			if (item.checkEvent("onBeforeChange")) {
				if (item.callEvent("onBeforeChange", [item._idd, item._value, item._newValue]) !== true) {
					...
					for (var q = 0; q < sel.options.length; q++)
						if (sel.options[q].value == item._value) sel.options[q].selected = true;
					return;
				}
			}
	...
	getValue: function(item) {
		...
		var opts = this.getOptions(item);
		for (var q = 0; q < opts.length; q++)
			if (opts[q].selected) k = opts[q].value;
		return k;
	},
	...
	_checkNoteWidth: function(item) {
		...
		if (item.childNodes[item._ll ? 1 : 0].childNodes[1] != null) {
			...
			if (t.className != null && t.className.search(/dhxform_note/gi) >= 0 && t._w == "auto") t.style.width = item.childNodes[
				item._ll ? 1 : 0].childNodes[0].offsetWidth + "px";
		}
		t = null;
	}
...
dhtmlXForm.prototype.items.multiselect = {
	...
	doOnChange: function(sel) {
		...
		item._newValue = [];
		for (var q = 0; q < sel.options.length; q++)
			if (sel.options[q].selected) item._newValue.push(sel.options[q].value);
	...
	getValue: function(item) {
		...
		var opts = this.getOptions(item);
		for (var q = 0; q < opts.length; q++)
			if (opts[q].selected) k.push(opts[q].value);
		return k;
	}
};
...
(function() {
	for (var a in dhtmlXForm.prototype.items.select) {
		if (!dhtmlXForm.prototype.items.multiselect[a]) dhtmlXForm.prototype.items.multiselect[a] = dhtmlXForm.prototype.items
			.select[a];
	}
})();
		...
		if (item._value != value) {
			if (item.checkEvent("onBeforeChange"))
				if (item.callEvent("onBeforeChange", [item._idd, item._value, value]) !== true) {
					// restore
					if (item._df != null) t.setValue(item, item._value);
					else item.childNodes[item._ll ? 1 : 0].childNodes[0].value = item._value;
					return;
				}
			// accepted
			if (item._df != null && foc) t.setValue(item, value);
			else item._value = value;
...
dhtmlXForm.prototype.items.button = {
	...
	render: function(item, data) {
		...
		item.className = String(item.className).replace("item_label_top", "item_label_left").replace("item_label_right",
			"item_label_left");
...
dhtmlXForm.prototype.items.fieldset = {
	...
	render: function(item, data) {
		...
		f.innerHTML = "<legend class='fs_legend' align='" + align + "' style='text-align:" + align + "'>" + data.label +
			"</legend>";
		item.appendChild(f);
...
dhtmlXForm.prototype.items.block = {
	...
	render: function(item, data) {
		...
		if (data.inputWidth != "auto")
			if (!isNaN(data.inputWidth)) b.style.width = parseInt(data.inputWidth) + "px";
	...
	for (var q = 0; q < ulData.childNodes.length; q++) {
		if (String(ulData.childNodes[q].tagName || "").toLowerCase() == "li") {
			...
			for (var w = 0; w < a.length; w++)
				if (t.getAttribute(a[w]) != null) p[String(a[w]).replace("ftype", "type")] = t.getAttribute(a[w]);
			if (!p.label) try {
...
dhtmlXForm.prototype.getUserData = function(id, name, rValue) {
	...
	if (this._userdata != null && typeof(this._userdata[id]) != "undefined" && typeof(this._userdata[id][name]) !=
		"undefined") return this._userdata[id][name];
	return "";
};
...
dhtmlXForm.prototype.setRTL = function(state) {
	...
	if (this._rtl) {
		...
		if (String(this.cont).search(/dhxform_rtl/gi) >= 0) this.cont.className = String(this.cont.className).replace(
			/dhxform_rtl/gi, "");
	}
};
...
dhtmlXForm.prototype.setFormData = function(t) {
	for (var a in t) {
		...
		switch (r) {
			case "checkbox":
				this[t[a] == true || parseInt(t[a]) == 1 || t[a] == "true" || t[a] == this.getItemValue(a, "realvalue") ?
					"checkItem" : "uncheckItem"](a);
				break;
			case "radio":
...
dhtmlXForm.prototype._ccActivate = function(id, inp, val) {
	...
	if (!this._formLS[id]) this._formLS[id] = {
		input: inp,
		value: val
	};
	if (!this._ccActive) {
...
dhtmlXForm.prototype._checkDim = function(formNode, inpObj) {
	...
	if (sNode != null) pNode.insertBefore(formNode, sNode);
	else pNode.appendChild(formNode);
...
(function() {
	...
	var a = [
		"ftype", "name", "value", "label", "check", "checked", "disabled", "text", "rows", "select", "selected", "width",
		"style", "className",
		"labelWidth", "labelHeight", "labelLeft", "labelTop", "inputWidth", "inputHeight", "inputLeft", "inputTop",
		"position", "size"
	];
...
if (typeof(window.dhtmlXCellObject) != "undefined") {
	...
	dhtmlXCellObject.prototype.attachForm = function(data) {
		...
		if (window.dhtmlx && dhtmlx.$customScroll) dhtmlx.CustomScroll.enable(obj);
